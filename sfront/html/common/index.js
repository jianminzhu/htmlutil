(function (require, $, SearchRenderUtil, JsonResultUtil, kkpager, juicer, md5, store, TplUtil) {
    $(function () {
        ...
        function sogou_params__deal__changeFiletypesToQuery(query, filetype) {
            ...
            return { filetype: filetype, query: query, queryShow: queryShow };
        }
        ...
        function getUrlAndHashParams() {
            ...
            return { urlParams: urlParams, hashParams: hashParams, needChagneUrl: needChagneUrl };
        }
        /*页面通用函数-------end*/
        require(["kendo.binder.min", "../ext/kui.ext.bindutil"], function (kendo, bindutil) {
            ...
            var cssClickDealFunction = {
                ...
                }, eClickQrest2: function (e) {
                    this.clean();
                    $("a.qreset2").hide();
                }, eChangeQuery: function (e) {
                    var jthis = $(e.target);
                    var query = jthis.val();
                    if (bypeLen(query) > this.queryLimitByteLen) {
                        var queryLimit80 = sub(query, this.queryLimitByteLen);
                        this.set("query", queryLimit80);
                        this.emsg(this.queryLimitInfo);
                    }
                }, eKeyupQuery: function (e) {
                    var jthis = $(e.target);
                    var query = jthis.val();
                    if (query != "") {
                        if (bypeLen(query) > this.queryLimitByteLen) {
                ...
                changeTime: function (inittime, isJustSearch) {
                    if (isJustSearch === void 0) { isJustSearch = true; }
                    var v = this;
                    this.set("page", "1");
                    ...
                    if (inittime != v.inittime) {
                        var inittimeMap = v.inittimemap[inittime];
                ...
                }, changeFiletypes: function (filetype) {
                    var query = this.query;
                    var filetype = filetype || "";
                    this.set("filetype", filetype);
                    var item = sogou_params__deal__changeFiletypesToQuery(query, filetype);
                    if ($.trim(item.query) != "") {
                        this.set("query", item["queryShow"]);
                    }
                }, eClickFiletype: function (e) {
                    var filetype = $(e.target).val();
                    this.changeFiletypes(filetype);
                    this.search();
                },
                changeRanktype: function (ranktype) {
                    this.set("page", 1);
                ...
                eClickInitTime: function (e) {
                ...
                }, setHashWithParams: function () {
                    var cacheKey = this.getCacheKey();
                    var params = bu.getParams(this, this.paramPropsForHashOrCache);
                    store.set("ca_" + cacheKey, params);
                    var hp = hashParams = bu.getParams(params, "cid,query");
                    window.location.hash = this.toHash([["cid", hp.cid], ["query", hp.query], ["ck", cacheKey]]);
                    return cacheKey;
                }, toHash: function (array2D) {
                    if (array2D === void 0) { array2D = []; }
                    var arr = [];
                    ...
                    if (v.isQueryNotEmpty()) {
                        try {
                            ...
                            var cacheObj = v.queryResultCache[cacheKey] || {};
                    ...
                    $(".emsg_box").hide();
                }, emsg: function (errorMsg, cleanHtml, hidePage) {
                    var v = this;
                    if (cleanHtml) {
                ...
                serverSearch: function (page, params, jout, paramMd5, uuid) {
                    var v = this;
                    return jsonpApi({
